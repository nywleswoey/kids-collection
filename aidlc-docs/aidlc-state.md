...
- **Current Stage**: INCREMENT 8 (New Eggs, Category Pick & Sort Fix) COMPLETE — deployed to Vercel production 2026-07-11 (dpl_Aog3CFUwB9TNfRCVaUpsiUpVqwtK, kids-collection.vercel.app). typecheck clean, 57/57 tests (+5 PBT), build ✅, zero new deps. FR1 common/rare egg (independent ~1% roll after epic+ miss; claim rarity gate removed — signed offer is the boundary); FR2 sacrifice-to-upgrade (3 copies → 50/50 same-vs-+1 tier, prefer-unowned, free, atomic; on card detail page when count≥3); FR3 category pick before pull (Random default, scopes normal draw only, eggs global); FR4 TRUE fix for grant-reshuffle — getAdminOverview was the unordered query (Inc7 only fixed listChildren); both now order by lower(name).
