...
- [x] PR #11 merged 2026-08-06 15:33Z — pnpm version pinned on main, so the 02:00 SGT schedule runs correctly. Vercel Production deploy success (commit status green).
- [x] `neondb_reader` DELETED from the Neon console — prod login roles now `cloud_admin, kc_backup_ro, neon_service, neondb_owner`. `kc_backup_ro` re-verified working after the removal.
- [x] Visual check of the profile-delete dialog — done by the user.
**INCREMENT 23 COMPLETE.** Parent OQ-B-1 (no backup/restore path) is now RESOLVED; §8 of the requirements lists 10 write-backs to Product-Definition that are NOT yet applied. Carried forward: **OQ-CS-3** (general delete-path PBT — parent's blocking-PBT constraint still unmet for the general case), **OQ-CS-2** (soft-delete → Increment 24; profile removal remains a hard cascading delete until then), **OQ-CS-4** (single vendor/account, accepted). Pre-existing drift recorded, out of scope: `test:pg` runs the contract suite on PG16 while prod is PG17.
